...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const loadSystemData = async () => {
    ...
    try {
      ...
      StorageRepository.setEvents(validEvents);

      // 3. Fetch Registrations (filter out deleted events)
      const rtdbRegs = await EventDatabaseService.getRegistrationsFromRealtimeDB();
      const validRegs = rtdbRegs.filter((r) => {
        const str = JSON.stringify(r).toLowerCase();
        return validEventIds.has(r.eventId) && !str.includes('john') && !str.includes('hackathon');
      });
      setRegistrations(validRegs);
      localStorage.setItem('eqa_registrations', JSON.stringify(validRegs));
  ...
  useEffect(() => {
    loadSystemData();
    const unsub = UserDatabaseService.subscribeToAllUsers((dbUsers) => {
      if (dbUsers && dbUsers.length > 0) {
        setUsers(dbUsers);
      }
    });
    const interval = setInterval(loadSystemData, 4000);
    return () => {
      unsub();
  ...
  // Quick Toggle Edit Lock
  const handleToggleProfileLock = async (targetUser: UserDatabaseRecord) => {
    try {
      const newOverride = !targetUser.allowEditOverride;
      await UserDatabaseService.adminUpdateUser(targetUser.id, {
        allowEditOverride: newOverride,
      });
      showNotification(
        newOverride
          ? `🔓 Edit lock removed for ${targetUser.name}.`
          : `🔒 Profile locked for ${targetUser.name}.`
      );
    } catch (err: any) {
      showNotification(null, err.message || 'Failed to update lock status.');
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deletingUser.role === 'admin' || deletingUser.email === 'admin@apexevents.in') {
      showNotification(null, 'The Master Admin account is protected and cannot be deleted.');
  ...
  const handleExportTabCSV = () => {
    if (activeTab === 'users') {
      CsvExportService.exportToCsv('Users_Database', filteredUsers, [
        { key: 'name', label: 'Full Name' },
        { key: 'email', label: 'Email' },
        { key: 'role', label: 'Role', formatter: (val) => String(val).toUpperCase() },
      ...
      CsvExportService.exportToCsv('Events_Database', events, [
      ...
      CsvExportService.exportToCsv('Platform_Feedback', portalFeedbacks, [
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-800 font-bold text-xs border border-slate-200 transition-all cursor-pointer shadow-xs"
                title={`Export ${activeTab.toUpperCase()} to CSV`}
              >
                        ...
                        <tr key={u.id} className="hover:bg-slate-50/60 transition-colors group">
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              {/* Unlock / Lock Profile Button */}
                              {u.role === 'attendee' && (
                                <button
                                  onClick={() => handleToggleProfileLock(u)}
                                  className={`p-2 rounded-xl border transition-all cursor-pointer ${
                                    u.allowEditOverride
                                      ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                                      : 'bg-slate-100 hover:bg-amber-50 hover:text-amber-700 text-slate-600 border-slate-200'
                                  }`}
                                  title={u.allowEditOverride ? "Profile Unlocked (Click to Re-lock)" : "Profile Locked (Click to Unlock)"}
                                >
              ...
              <div className="space-y-1.5">
                ...
                {editingUser.role === 'admin' ? (
                  <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-between">
                    <span className="text-xs font-black text-purple-900">🛡️ Master Administrator</span>
                    <span className="text-[11px] font-bold text-purple-600">Fixed System Role</span>
                  </div>
                ) : (
                  <div className="grid grid-cols-3 gap-2">
                    {(['teacher', 'organizer', 'attendee'] as UserRole[]).map((r) => (
                      <button
                        key={r}
                        type="button"
                ...
                <input
                  ...
                  className={`w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-semibold focus:outline-none ${
                    editingUser.role === 'admin'
                      ...
                      : 'bg-slate-50 focus:bg-white focus:ring-2 focus:ring-purple-500'
                  }`}
                />
              </div>
