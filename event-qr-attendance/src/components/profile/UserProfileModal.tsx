...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  // Check Edit Lock Status: Locked permanently once Name & Registration Number are set
  const isProfileSet = Boolean(currentUser.registrationNumber && currentUser.name);
      ...
      <div className="bg-white rounded-3xl max-w-xl w-full shadow-2xl border border-slate-200 overflow-hidden my-auto max-h-[92vh] flex flex-col transform transition-all">
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-4 sm:px-6 pt-2 shrink-0">
          <button
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2.5 text-xs font-black border-b-2 transition-all cursor-pointer ${
              activeTab === 'profile'
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-2.5 pl-10 text-xs font-mono font-bold uppercase focus:outline-none ${
                      isLocked
                        ? 'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed'
                        : 'bg-slate-50 border-slate-200 text-slate-800 focus:border-indigo-600'
                    }`}
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-2xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer ${
                  isLocked
                    ...
                    : 'bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 text-white shadow-lg shadow-indigo-500/20 hover:scale-[1.01]'
                }`}
              >
                {isLoading
                  ? 'Saving Details...'
                  : isLocked
                  ? '🔒 Profile Details Permanently Locked'
