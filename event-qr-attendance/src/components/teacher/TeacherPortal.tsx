...
export const TeacherPortal: React.FC<TeacherPortalProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const userMap = new Map<string, UserDatabaseRecord>();
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-slate-200 px-4 sm:px-8 py-3.5 shadow-sm">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-amber-500 via-orange-500 to-amber-600 flex items-center justify-center shadow-md shadow-amber-500/20 text-white font-black">
              <GraduationCap className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xl font-black tracking-tight text-slate-900">Faculty Portal</span>
                <span className="px-2 py-0.5 rounded-md bg-amber-100 text-amber-900 font-mono text-[10px] font-black uppercase tracking-wider border border-amber-200">
                  Teacher View
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {currentUser.role === 'admin' && onSwitchPortalView && (
              <button
                ...
                className="hidden sm:inline-flex px-3 py-1.5 rounded-xl bg-purple-100 hover:bg-purple-200 text-purple-900 text-xs font-bold transition-all cursor-pointer"
              >
                ← Back to Admin
              </button>
            )}
            ...
            <button
              onClick={loadData}
              disabled={isLoading}
              className="p-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 hover:text-amber-600 shadow-sm transition-all cursor-pointer"
            ...
            <div className="flex items-center gap-2.5 pl-2 border-l border-slate-200">
              <img
                ...
                className="w-9 h-9 rounded-xl object-cover border-2 border-amber-200 shadow-sm"
              />
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-8 pt-8 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="max-w-2xl mx-auto pt-4">
            <div className="relative shadow-lg rounded-3xl overflow-hidden border-2 border-amber-200 focus-within:border-amber-500 transition-all bg-white">
              <Search className="w-5 h-5 text-amber-600 absolute left-5 top-1/2 -translate-y-1/2" />
              <input
                type="text"
                autoFocus
                placeholder="Enter Student Register Number (e.g. REG-2026-884, 21B91A...)"
                value={searchQuery}
                onChange={(e) => {
                ...
                className="w-full pl-14 pr-12 py-4 text-sm sm:text-base font-bold text-slate-900 placeholder:text-slate-400 placeholder:font-normal focus:outline-none bg-transparent"
              />
              {searchQuery && (
                <button
                  onClick={() => {
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 cursor-pointer font-bold text-xs"
                >
                  ✕ Clear
                </button>
              )}
            ...
            <div className="flex items-center justify-center gap-2 pt-3 text-[11px] text-slate-500">
              ...
              {allUsers
                ...
                .map((sampleUser) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200 font-mono font-bold cursor-pointer transition-all"
                  >
