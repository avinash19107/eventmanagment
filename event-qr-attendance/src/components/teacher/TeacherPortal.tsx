...
export const TeacherPortal: React.FC<TeacherPortalProps> = ({
  ...
  const handleExportStudentReport = () => {
    if (!selectedStudent || studentRegistrations.length === 0) {
      alert('No event participation records found for this student.');
      return;
    }

    const eventMap = new Map(allEvents.map((e) => [e.id, e]));

    const exportData = studentRegistrations.map((r) => {
      const evt = eventMap.get(r.eventId);
      const isAttended = isCheckedInReg(r);
      return {
        studentName: selectedStudent.name,
        studentEmail: selectedStudent.email,
        registrationNumber: selectedStudent.registrationNumber || 'N/A',
        eventTitle: evt?.title || 'Unknown Event',
        category: evt?.category?.toUpperCase() || 'EVENT',
        eventDate: evt?.startDate || 'N/A',
        ticketReference: r.reference,
        status: isAttended ? 'Verified Attended' : r.status.toUpperCase(),
        checkedInAt: r.checkedInAt || (isAttended ? 'Checked In' : 'Not Checked In'),
      };
    });

    CsvExportService.exportToCsv(
      `Attendance_Report_${selectedStudent.registrationNumber || selectedStudent.name.replace(/\s+/g, '_')}`,
      exportData,
      [
        ...
        { key: 'ticketReference', label: 'Ticket Reference' },
        { key: 'status', label: 'Attendance Status' },
        { key: 'checkedInAt', label: 'Check-In Timestamp' },
      ]
    );
  };
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-slate-100">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 active:scale-95 text-white text-xs font-bold cursor-pointer transition-all shadow-sm"
                    title="Export Student Attendance as CSV"
                  >
