...
export const OrganizerPortal: React.FC<OrganizerPortalProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-600 selection:text-white pb-24">
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-base sm:text-xl font-black tracking-tight text-slate-900 truncate">Organizer Portal</span>
                ...
                <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200 truncate max-w-[140px] sm:max-w-none">
                  {currentUser.email}
                </span>
              </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <button
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl sm:rounded-2xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 font-extrabold text-xs transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl sm:rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-black text-xs shadow-md shadow-indigo-600/20 transition-all cursor-pointer"
            >
            ...
            <button
              onClick={onSignOut}
              title="Sign Out"
              className="p-2 rounded-xl sm:rounded-2xl bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-600 border border-slate-200 shadow-sm transition-all cursor-pointer shrink-0"
            >
      ...
      {/* 2. STATS STRIP */}
      <section className="max-w-7xl mx-auto px-3 sm:px-8 pt-4 sm:pt-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4">
          <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm space-y-1">
            <div className="flex items-center justify-between text-[11px] sm:text-xs font-extrabold text-slate-400 uppercase">
              <span>Active Events</span>
              <Calendar className="w-4 h-4 text-indigo-600" />
            </div>
            <p className="text-xl sm:text-2xl font-black text-slate-900">{activeEvents.length}</p>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">{endedEvents.length} ended</p>
          </div>
          ...
          <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm space-y-1">
            ...
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">Across all events</p>
          </div>
          ...
          <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm space-y-1">
            ...
            <p className="text-xl sm:text-2xl font-black text-emerald-600">{totalCheckIns}</p>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">Gate verifications</p>
          </div>
          ...
          <div className="bg-white p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm space-y-1">
            ...
            <p className="text-xs sm:text-sm font-black text-slate-900 truncate">{currentUser.name}</p>
            <p className="text-[10px] sm:text-[11px] text-indigo-600 font-bold truncate">{currentUser.email}</p>
          </div>
        </div>
      </section>

      {/* 3. TWO COLUMN WORKSPACE */}
      <main className="max-w-7xl mx-auto px-3 sm:px-8 pt-4 sm:pt-8 space-y-6 sm:space-y-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          {/* LEFT: QR CODE ATTENDANCE SCANNER TOOL (5 COLS) */}
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white p-5 sm:p-8 rounded-3xl border border-slate-200/90 shadow-xl space-y-5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black shrink-0 shadow-md shadow-indigo-600/20">
                    <QrCode className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <div>
                    <h3 className="text-base sm:text-lg font-black text-slate-900 tracking-tight">QR Ticket Pass Scanner</h3>
                    <p className="text-[11px] sm:text-xs text-slate-500 font-medium">Verify &amp; mark attendance at gate</p>
                  </div>
                </div>
              ...
              {isCameraActive && (
                <div className="p-3 bg-slate-900 rounded-2xl space-y-2 animate-fade-in">
                  <div id="mobile-qr-reader" className="overflow-hidden rounded-xl border border-slate-700" />
                  <p className="text-[10px] text-slate-300 text-center font-medium">
                    Position attendee QR ticket code inside camera frame to auto check-in.
                  </p>
                </div>
              )}

              <form onSubmit={handleVerifyCheckIn} className="space-y-4 pt-1">
                <div>
                  <label className="block text-[11px] sm:text-xs font-extrabold text-slate-700 uppercase mb-1.5">
          ...
          <div className="lg:col-span-7 space-y-6">
            {/* Tab Bar - Mobile Optimized Labels */}
            <div className="flex items-center gap-1.5 sm:gap-2 bg-slate-100 rounded-2xl p-1.5">
              {[
                { id: 'active', label: 'Active', icon: <Zap className="w-3.5 h-3.5" />, count: activeEvents.length },
                { id: 'ended', label: 'Previous', icon: <History className="w-3.5 h-3.5" />, count: endedEvents.length },
                { id: 'admin', label: 'Users', icon: <Shield className="w-3.5 h-3.5" />, count: allUserRecords.length },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => handleTabChange(tab.id as any)}
                  className={`flex-1 flex items-center justify-center gap-1 sm:gap-1.5 py-2 px-2 sm:px-3 rounded-xl text-xs font-extrabold transition-all cursor-pointer ${
                    activeTab === tab.id
