...
import { Event, User, Registration, AttendanceRecord, EventWinner, EventFeedback } from '../../types';
...
interface OrganizerFeedbackViewerModalProps {
  event: Event;
  onClose: () => void;
}

const OrganizerFeedbackViewerModal: React.FC<OrganizerFeedbackViewerModalProps> = ({ event, onClose }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      const localFb = StorageRepository.getFeedback(event.id);
      const rtdbFb = await EventDatabaseService.getFeedbackForEvent(event.id);
      const map = new Map<string, EventFeedback>();
      localFb.forEach((f) => map.set(f.id, f));
      rtdbFb.forEach((f) => map.set(f.id, f));
      setFeedbackList(Array.from(map.values()));
      setIsLoading(false);
    };
    load();
  }, [event.id]);
  ...
  const avgMentorship =
    feedbackList.length > 0
      ? (feedbackList.reduce((acc, f) => acc + (f.mentorshipRating || 5), 0) / feedbackList.length).toFixed(1)
      : '5.0';

  const avgOrg =
    feedbackList.length > 0
      ? (feedbackList.reduce((acc, f) => acc + (f.organizationRating || 5), 0) / feedbackList.length).toFixed(1)
      : '5.0';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-slate-200 overflow-hidden my-auto max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white flex items-start justify-between shrink-0">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h2 className="text-lg font-black text-white leading-tight mt-1">{event.title}</h2>
              <p className="text-xs text-indigo-200">Attendee feedback submitted to unlock certificates</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 active:scale-95 text-white flex items-center justify-center transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {/* Stats Summary Strip */}
        <div className="grid grid-cols-3 divide-x divide-slate-100 bg-slate-50 border-b border-slate-200/80 p-4 text-center shrink-0">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase">Avg Experience</span>
          ...
          <div>
            ...
            <p className="text-lg font-black text-indigo-600 flex items-center justify-center gap-1">
              <span>{avgMentorship}</span>
              <Star className="w-4 h-4 fill-indigo-400 text-indigo-500" />
            </p>
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase">Organization</span>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-3.5">
          {isLoading ? (
            <div className="py-12 text-center text-slate-400 text-xs font-medium">Loading feedback...</div>
          ) : feedbackList.length === 0 ? (
            <div className="py-12 text-center text-slate-400 text-xs font-medium bg-slate-50 rounded-2xl border border-slate-200/60 p-6 space-y-1">
              <MessageSquare className="w-8 h-8 text-slate-300 mx-auto mb-2" />
              <p className="font-bold text-slate-700">No Feedback Submitted Yet</p>
              <p className="text-[11px] text-slate-400">Attendees will submit feedback to verify &amp; access their certificates.</p>
            </div>
          ) : (
            feedbackList.map((fb) => (
              <div key={fb.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200/90 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">{fb.userEmail}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-full text-amber-900 text-xs font-black">
                    <span>{fb.rating}</span>
...
export const OrganizerPortal: React.FC<OrganizerPortalProps> = ({
  ...
  const handleExportAllRegistrations = () => {
    ...
    const exportData = allRegistrations.map((r) => {
      const evt = eventMap.get(r.eventId);
      const isCheckedIn =
        r.status === 'checked_in' ||
        !!r.checkedInAt ||
        allAttendance.some((a) => a.registrationId === r.id);
      return {
        reference: r.reference,
        eventTitle: evt?.title || 'Unknown Event',
        attendeeName: r.attendeeName,
        attendeeEmail: r.attendeeEmail,
        registrationNumber: r.registrationNumber || 'N/A',
        teamName: r.teamName || 'Individual',
        status: isCheckedIn ? 'CHECKED_IN' : r.status.toUpperCase(),
        checkedInAt: r.checkedInAt || (isCheckedIn ? 'Checked In' : 'Pending'),
        registeredAt: r.createdAt || 'N/A',
      };
    });

    CsvExportService.exportToCsv('Organizer_Registrations', exportData, [
      { key: 'reference', label: 'Ticket Reference' },
      { key: 'eventTitle', label: 'Event Name' },
      { key: 'attendeeName', label: 'Attendee Name' },
      { key: 'attendeeEmail', label: 'Email' },
      { key: 'registrationNumber', label: 'Register ID' },
  ...
  const loadData = async () => {
    try {
      ...
      const myRegs = (isGlobalAdmin ? regs : regs.filter((r) => myEventIds.has(r.eventId))).filter((r) => {
  ...
  const handleVerifyCheckIn = async (e: React.FormEvent) => {
    ...
    // Record Check-in locally and in Firebase RTDB
    const newRecord: AttendanceRecord = {
    ...
    StorageRepository.saveAttendanceRecord(newRecord);
    await EventDatabaseService.saveAttendanceToRealtimeDB(newRecord);
    await EventDatabaseService.checkInRegistration(reg.id, currentUser.name);
    ...
    StorageRepository.saveRegistration(updatedReg);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-600 selection:text-white pb-24">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-8 pt-4 sm:pt-8 space-y-6 pb-16">
        {/* Quick Tab Switcher for Mobile & Tablet */}
        <div className="flex md:hidden items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none py-1">
          <button
            onClick={() => handleTabChange('active')}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-xs font-bold whitespace-nowrap shrink-0 transition-all active:scale-95 ${
              activeTab === 'active'
          ...
          >
            ...
            <span>Ended ({endedEvents.length})</span>
          </button>
          <button
          ...
          >
            ...
            <span>Roster &amp; Heatmap ({allRegistrations.length})</span>
          </button>
        </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs border border-slate-200 transition-all cursor-pointer shadow-xs active:scale-95"
                      ...
                      >
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          ...
                          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                            <button
                              onClick={() => setFeedbackModalEvent(evt)}
                              className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-2xl bg-indigo-50 hover:bg-indigo-100 active:scale-95 text-indigo-700 border border-indigo-200 text-xs font-black transition-all cursor-pointer"
                            >
      ...
      {/* ORGANIZER PARTICIPANT FEEDBACK MODAL */}
      {feedbackModalEvent && (
        <OrganizerFeedbackViewerModal
