...
export const OrganizerPortal: React.FC<OrganizerPortalProps> = ({
  ...
  const loadData = async () => {
    try {
      const [rtdbEvents, users, regs] = await Promise.all([
        EventDatabaseService.getEventsFromRealtimeDB(),
        UserDatabaseService.getAllUsersFromRealtimeDB(),
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 hover:bg-purple-100 text-xs font-bold transition-all cursor-pointer shadow-sm flex items-center gap-1"
                            >
                            ...
                            <button
                              ...
                              title="Edit Event Details (Venue, Timings, Title)"
                              className="p-2 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 transition-all cursor-pointer"
                            >
        ...
        {activeTab === 'admin' && (
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/90 shadow-xl space-y-6">
              ...
              {unlockNotice && (
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-bold flex items-center justify-between animate-fade-in">
              ...
              {/* User Account Records & Profile Unlock */}
              {allUserRecords.length > 0 && (
                <div className="pt-6 border-t border-slate-200 space-y-4">
                  <h4 className="text-sm font-black text-slate-900 uppercase tracking-wider">
                    Registered User Accounts &amp; Admin Override Locks
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {allUserRecords.map((usr) => {
                      const isLocked = Boolean(usr.registrationNumber && usr.name && !usr.allowEditOverride);
                      return (
                        <div
                          key={usr.id}
                        ...
                        >
                          <div className="min-w-0 space-y-0.5">
                            <p className="font-extrabold text-slate-900 truncate">{usr.name}</p>
                            <p className="text-slate-500 text-[11px] truncate">{usr.email}</p>
                            <p className="font-mono text-[10px] font-bold text-indigo-700">
                              ID: {usr.registrationNumber || 'Not Set'}
                            </p>
                          </div>
                          {isLocked ? (
                            <button
                              onClick={() => handleUnlockUserEdit(usr)}
                              title="Unlock profile edit for this user"
                              className="px-2.5 py-1.5 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 font-extrabold text-[10px] transition-all cursor-pointer shrink-0"
                            >
                              🔓 Unlock Edit
                            </button>
                          ) : (
