...
export const OrganizerRegisterMemberModal: React.FC<OrganizerRegisterMemberModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent, addAnother = false) => {
    ...
    try {
      ...
      // Save user account to Firebase RTDB & LocalStorage without resetting existing passwords
      if (existingUserRecord && existingUserRecord.password) {
        await UserDatabaseService.saveUserToRealtimeDB(userAccount, 'password', existingUserRecord.password);
      } else {
        const tempPass = `Apex@${Math.floor(100000 + Math.random() * 900000)}`;
        await UserDatabaseService.saveUserToRealtimeDB(userAccount, 'password', tempPass);
      }
        ...
        <form onSubmit={(e) => handleSubmit(e, false)} className="p-6 sm:p-8 overflow-y-auto space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Alex Rivera / Rahul Sharma"
