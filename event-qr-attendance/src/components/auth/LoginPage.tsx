...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  // Desktop mode form panel visibility state (default false = 100% full screen desktop video view)
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-0 font-sans selection:bg-purple-600 selection:text-white relative overflow-hidden">
      {/* Desktop Top Right Glowing "Sign In" Button */}
      {!isDesktopFormOpen && (
        <button
          onClick={() => setIsDesktopFormOpen(true)}
          className="hidden md:flex fixed top-8 right-10 z-30 items-center gap-2.5 px-7 py-3.5 rounded-full bg-white/15 hover:bg-white/30 backdrop-blur-md text-white font-black text-sm border border-white/40 shadow-2xl hover:border-white/70 transition-all cursor-pointer hover:scale-105 group animate-fade-in"
        >
          <LogIn className="w-4 h-4 text-purple-300 group-hover:text-white transition-colors" />
          <span>Sign In / Access Portal →</span>
        </button>
      )}

      {/* Outer Container: Mobile Stacked vs Desktop Full-Screen Viewport */}
      <div className="w-full min-h-screen md:max-w-none md:min-h-screen bg-slate-950 rounded-none shadow-none overflow-y-auto flex flex-col md:flex-row relative transition-all m-0 p-0">
        
        {/* Banner Panel: Mobile Header vs Laptop 100% Full-Screen Video Banner */}
        <div className="relative w-full md:w-full md:fixed md:inset-0 md:h-screen h-44 sm:h-56 bg-gradient-to-br from-[#7C5CFC] via-[#6846EC] to-[#4F2FD4] p-5 sm:p-8 md:p-16 lg:p-24 flex flex-col justify-between text-white overflow-hidden select-none rounded-none shrink-0 z-0">
          ...
          {/* Left Text Layered Cleanly ON TOP OF Full-Screen Video Background */}
          <div className="relative z-20 space-y-2 sm:space-y-4 my-auto py-2 sm:py-6 max-w-xl">
            <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-7xl font-black tracking-tight text-white leading-tight drop-shadow-2xl">
              {pageView === 'signin'
        ...
        {/* Form Container: Mobile Stacked vs Desktop Floating Right Side Drawer Panel */}
        <div
          className={`w-full md:w-[460px] lg:w-[520px] md:fixed md:top-0 md:right-0 md:h-screen md:z-40 bg-white md:bg-white/95 md:backdrop-blur-2xl md:shadow-2xl p-6 sm:p-10 md:p-14 flex flex-col justify-between relative rounded-b-[32px] md:rounded-none overflow-y-auto transition-all duration-500 transform ${
            isDesktopFormOpen ? 'md:translate-x-0' : 'md:translate-x-full md:pointer-events-none'
          }`}
        >
          ...
          <button
            ...
            className="hidden md:flex absolute top-6 right-6 p-2.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 transition-all cursor-pointer shadow-sm z-50"
            title="Close Sign In Panel (Return to Full Screen Video)"
          >
            <X className="w-5 h-5" />
          </button>
