...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-screen bg-slate-950 flex items-center justify-center p-0 font-sans selection:bg-purple-600 selection:text-white relative overflow-hidden select-none">
      
      {/* 100% Full-Screen Video Background Container for BOTH Desktop & Mobile */}
      <div className="fixed inset-0 w-full h-screen z-0 overflow-hidden bg-slate-950">
      ...
      <div
        ...
        } bottom-0 left-0 right-0 max-h-[85vh] rounded-t-[32px] p-6 sm:p-8 overflow-y-auto md:top-0 md:bottom-0 md:right-0 md:left-auto md:w-[480px] lg:w-[520px] md:h-screen md:max-h-none md:rounded-none md:p-12 flex flex-col justify-between`}
      >
        {/* Close Button (Desktop & Mobile) */}
        <button
          type="button"
          onClick={() => setIsFormOpen(false)}
          className="absolute top-5 right-5 p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 transition-all cursor-pointer shadow-sm z-50"
            ...
            {pageView === 'signin' && (
              <>
                ...
                <div className="pt-3 border-t border-slate-100 space-y-2">
                  <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest block text-center font-bold">
                    Quick Role Sign-In Preview
                  </span>
                  <div className="grid grid-cols-3 gap-1.5 text-[10px] font-bold">
                    <button
                      type="button"
                      onClick={() => {
                        setUsernameOrEmail('admin@apexevents.in');
                        setPassword('admin123');
                        setErrorMsg(null);
                      }}
                      className="py-1.5 px-2 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-900 border border-purple-200 transition-all cursor-pointer text-center"
                    >
                      🛡️ Admin
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setUsernameOrEmail('teacher@apexevents.in');
                        setPassword('teacher123');
                        setErrorMsg(null);
                      }}
                      className="py-1.5 px-2 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200 transition-all cursor-pointer text-center"
                    >
                      🎓 Teacher
                    </button>
                    <button
                      ...
                      className="py-1.5 px-2 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-900 border border-indigo-200 transition-all cursor-pointer text-center"
                    >
                      🎪 Organizer
                    </button>
                  </div>
