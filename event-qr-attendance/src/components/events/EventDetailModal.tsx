      ...
      if (event.format === 'team') {
        if (teamActionType === 'create_full') {
          ...
          for (let i = 0; i < validTeammates.length; i++) {
            ...
            const mRef = `REF-${Math.floor(100000 + Math.random() * 900000)}`;
          ...
          if (!targetTeam) {
            alert('Could not find the specified team. Please check the code or selection.');
            setIsLoading(false);
            return;
          }
                ...
                {event.format === 'team' && (
                  <div className="space-y-3 p-3.5 rounded-xl bg-white border border-indigo-200">
                    ...
                    {teamActionType === 'create_full' ? (
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-slate-600 uppercase mb-1">Enter Team Join Code or Select Team</label>
                        <input
                          type="text"
                          value={joinCodeInput}
                          onChange={(e) => setJoinCodeInput(e.target.value.toUpperCase())}
                          placeholder="e.g. TEAM-4921"
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono font-bold text-purple-900 uppercase focus:outline-none focus:border-purple-600 mb-2"
                        />
                          ...
                          >
                            ...
                            {registeredTeams.map((t) => (
                              <option key={t.name} value={t.name} disabled={t.members.length >= maxTeamSize}>
                                {t.name} ({t.members.length}/{maxTeamSize} Members)
                              </option>
                            ))}
                          </select>
                        )}
