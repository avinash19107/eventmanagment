  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      if (event.format === 'team') {
        ...
        const currentCount = allRegistrations.filter((r) => r.eventId === event.id && r.status !== 'cancelled').length;
        const isTeamWaitlisted = currentCount >= event.maxCapacity;

        if (currentCount + totalTeamMembers > event.maxCapacity && !event.waitlistEnabled) {
          alert(`This event only has ${Math.max(0, event.maxCapacity - currentCount)} spot(s) remaining, but your team has ${totalTeamMembers} members.`);
          setIsLoading(false);
          return;
        }

        const teamRegStatus = isTeamWaitlisted ? 'waitlisted' : 'confirmed';
        ...
        const leaderReg: Registration = {
          id: `reg-${Date.now()}-${Math.random().toString(36).substring(2, 7)}-ldr`,
          eventId: event.id,
        ...
        StorageRepository.saveCredential({
          id: `cred-${Date.now()}-${Math.random().toString(36).substring(2, 7)}-0`,
        ...
        for (let i = 0; i < validTeammates.length; i++) {
          ...
          const mReg: Registration = {
            id: `reg-${Date.now()}-${Math.random().toString(36).substring(2, 7)}-m${i + 1}`,
            eventId: event.id,
            reference: mRef,
          ...
          StorageRepository.saveCredential({
            id: `cred-${Date.now()}-${Math.random().toString(36).substring(2, 7)}-${i + 1}`,
