...
export const UserEventPortal: React.FC<UserEventPortalProps> = ({
  ...
  const [selectedCertEvent, setSelectedCertEvent] = useState<Event | null>(null);
  const [selectedCertWinner, setSelectedCertWinner] = useState<EventWinner | null>(null);
  const [selectedCertRef, setSelectedCertRef] = useState<string>('');
  const [isCertModalOpen, setIsCertModalOpen] = useState(false);

  const handleOpenCertificate = (evt: Event, regRef?: string) => {
    const winners = allWinnersMap[evt.id] || [];
    const myWinnerInfo = winners.find(
      (w) => w.attendeeEmail.toLowerCase() === currentUser.email.toLowerCase()
    ) || null;

    setSelectedCertEvent(evt);
    setSelectedCertWinner(myWinnerInfo);
    setSelectedCertRef(regRef || '');
    setIsCertModalOpen(true);
  };

  const handleDeletePass = async (regId: string, evtTitle?: string) => {
    const confirmed = window.confirm(
      `Delete ticket pass record for "${evtTitle || 'this event'}"?`
    );
    if (confirmed) {
      StorageRepository.deleteRegistration(regId);
      setMyRegistrations((prev) => prev.filter((r) => r.id !== regId));
      setAllRegistrations((prev) => prev.filter((r) => r.id !== regId));
      addToast('info', 'Ticket pass record deleted successfully.');
    }
  };
              ...
              isRegistered ? (
                <button
                  onClick={(e) => {
                    ...
                    handleOpenCertificate(evt, myReg?.reference);
                  }}
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-600 selection:text-white pb-24">
                  ...
                  {myRegistrations.map((reg) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="pt-2 flex items-center justify-between border-t border-slate-200/80 text-[11px] text-slate-500 gap-2 flex-wrap">
                          <span>{new Date(reg.createdAt).toLocaleDateString()}</span>
                          <div className="flex items-center gap-2">
                            {evt && (evt.status === 'ended' || evt.status === 'archived') ? (
                              <>
                                <button
                                  onClick={() => handleOpenCertificate(evt, reg.reference)}
                                  className="text-amber-700 font-extrabold hover:underline flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded border border-amber-200"
                                >
                                  <Award className="w-3 h-3 text-amber-600" />
                                  <span>Certificate</span>
                                </button>
                                <button
                                  onClick={() => handleDeletePass(reg.id, evt.title)}
                                  className="text-rose-600 font-extrabold hover:underline flex items-center gap-1 bg-rose-50 px-2 py-0.5 rounded border border-rose-200"
      ...
      />
      {/* 9. DIGITAL CERTIFICATE MODAL */}
      <CertificateModal
        event={selectedCertEvent}
        currentUser={currentUser}
        winnerInfo={selectedCertWinner}
        registrationRef={selectedCertRef}
