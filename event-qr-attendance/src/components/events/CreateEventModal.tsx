...
export const CreateEventModal: React.FC<CreateEventModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-slate-200 overflow-hidden my-auto max-h-[92vh] flex flex-col transform transition-all">
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 p-4 sm:p-6 text-white flex items-center justify-between relative shadow-md shrink-0">
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-100 bg-white/20 px-2.5 py-0.5 sm:py-1 rounded-full border border-white/25">
              Event Management
            </span>
            <h2 className="text-lg sm:text-2xl font-black mt-1 tracking-tight">Create &amp; Publish New Event</h2>
            <p className="text-xs text-indigo-100 font-medium hidden sm:block">Fill in the details below to publish your event</p>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 active:scale-95 flex items-center justify-center text-white transition-colors cursor-pointer shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 hover:bg-indigo-50/70 transition-all">
              ...
              <div className="flex-1 space-y-1.5 text-center sm:text-left">
                ...
                <p className="text-[11px] text-slate-500 font-medium">
                  PNG, JPG, or WEBP. Upload your custom event banner or choose a preset below.
                </p>
              </div>
            </div>
                ...
                {PRESET_BANNERS.map((url, idx) => (
                  <button
                    ...
                    className={`relative rounded-xl overflow-hidden h-14 border-2 transition-all cursor-pointer ${
                      bannerUrl === url && !customPhotoSelected
                        ? 'border-indigo-600 ring-2 ring-indigo-400/40 shadow-sm'
