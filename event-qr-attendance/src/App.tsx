  ...
  useEffect(() => {
    ...
    const verifyDatabaseRole = async () => {
      try {
        ...
        if (dbRecord) {
          if (dbRecord.status === 'inactive') {
            console.warn('🔒 Account Deactivated: Logging user out.');
            handleSignOut();
            setSecurityNotice('⛔ Your account has been deactivated. Please contact an administrator.');
            return;
          }
