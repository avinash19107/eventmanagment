...
export class EventDatabaseService {
  ...
  /**
   * Fetch all winners across all events directly from Firebase Realtime Database
   */
  public static async getAllWinnersFromRealtimeDB(): Promise<Record<string, EventWinner[]>> {
    try {
      const winnersRef = ref(rtdb, 'winners');
      const snapshot = await get(winnersRef);
      if (snapshot.exists()) {
        const raw = snapshot.val() as Record<string, Record<string, EventWinner>>;
        const result: Record<string, EventWinner[]> = {};
        Object.entries(raw).forEach(([eventId, winnersObj]) => {
          if (winnersObj && typeof winnersObj === 'object') {
            result[eventId] = Object.values(winnersObj);
          }
        });
        return result;
      }
    } catch (err) {
      console.warn('Firebase RTDB Fetch All Winners Notice:', err);
    }
    return {};
  }
  ...
  /**
   * Update Team Logo URL across all registrations for an event in Firebase Realtime Database
   */
  public static async updateTeamLogoInRealtimeDB(
    eventId: string,
    teamName: string,
    newLogoUrl: string
  ): Promise<void> {
    try {
      ...
      if (snapshot.exists()) {
        ...
        Object.entries(data).forEach(([key, r]) => {
          if (r && r.eventId === eventId && r.teamName === teamName) {
            updates.push(
              update(ref(rtdb, `registrations/${key}`), {
      ...
      console.warn('Firebase RTDB Team Logo Update Notice:', err);
    }
  }

  /**
   * Save Event Feedback to Firebase Realtime Database
   */
  public static async saveFeedbackToRealtimeDB(feedback: any): Promise<void> {
    try {
      const fbRef = ref(rtdb, `feedback/${feedback.eventId}/${feedback.id}`);
      await set(fbRef, {
        ...feedback,
        submittedAt: feedback.submittedAt || new Date().toISOString(),
      });
    } catch (err) {
      console.warn('Firebase RTDB Feedback Save Notice:', err);
    }
  }
   ...
   */
  public static async getFeedbackForEvent(eventId: string): Promise<any[]> {
    try {
      const fbRef = ref(rtdb, `feedback/${eventId}`);
      ...
      console.warn('Firebase RTDB Fetch Event Feedback Notice:', err);
    }
    return [];
  }

  /**
   * Fetch all feedback across all events
   */
  public static async getAllFeedbackFromRealtimeDB(): Promise<any[]> {
    try {
      const fbRef = ref(rtdb, 'feedback');
      const snapshot = await get(fbRef);
      if (snapshot.exists()) {
        const data = snapshot.val() as Record<string, Record<string, any>>;
        const allFeedback: any[] = [];
        Object.values(data).forEach((eventFb) => {
          if (eventFb && typeof eventFb === 'object') {
            allFeedback.push(...Object.values(eventFb));
          }
        });
        return allFeedback;
      }
    } catch (err) {
      console.warn('Firebase RTDB Fetch All Feedback Notice:', err);
    }
    return [];
  }

  /**
   * Save ApexEvents Platform / Portal Feedback to Firebase Realtime Database
   */
  public static async savePlatformFeedbackToRealtimeDB(feedback: any): Promise<void> {
    try {
      const pfRef = ref(rtdb, `portalFeedback/${feedback.id}`);
      await set(pfRef, {
        ...feedback,
        createdAt: feedback.createdAt || new Date().toISOString(),
      });
    } catch (err) {
      console.warn('Firebase RTDB Platform Feedback Save Notice:', err);
    }
  }

  /**
   * Fetch all Platform / Portal Feedback from Firebase Realtime Database
   */
  public static async getPlatformFeedbackFromRealtimeDB(): Promise<any[]> {
    try {
      const pfRef = ref(rtdb, 'portalFeedback');
      const snapshot = await get(pfRef);
      if (snapshot.exists()) {
        const data = snapshot.val() as Record<string, any>;
        return Object.values(data);
      }
    } catch (err) {
      console.warn('Firebase RTDB Platform Feedback Fetch Notice:', err);
    }
    return [];
  }

  /**
   * Delete Platform Feedback from Firebase Realtime Database
   */
  public static async deletePlatformFeedbackFromRealtimeDB(feedbackId: string): Promise<void> {
    try {
      const pfRef = ref(rtdb, `portalFeedback/${feedbackId}`);
      await remove(pfRef);
    } catch (err) {
      console.warn('Firebase RTDB Platform Feedback Delete Notice:', err);
    }
  }
