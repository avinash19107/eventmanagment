...
export class EventDatabaseService {
  ...
  /**
   * Delete a registration from Firebase Realtime Database
   */
  public static async deleteRegistrationFromRealtimeDB(registrationId: string): Promise<void> {
    try {
      const regRef = ref(rtdb, `registrations/${registrationId}`);
      await remove(regRef);
    } catch (err) {
      console.warn('Firebase RTDB Delete Registration Notice:', err);
    }
  }
