...
export class UserDatabaseService {
  ...
  public static async updateUserProfileDetails(
    ...
    const allowOverride = Boolean(existing?.allowEditOverride || user.allowEditOverride);

    // Rule: Can only edit once UNLESS Admin enabled allowEditOverride
    if (editCount >= 1 && !allowOverride) {
      throw new Error('🔒 Profile details are locked after your 1-time edit. An Admin must unlock your account to make further changes.');
    }
    ...
    const updatedUser: User = {
      ...
      allowEditOverride: false, // Consume the unlock override once used
    };
