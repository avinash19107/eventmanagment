...
export class AuthService {
  ...
  public static async loginWithCredentials(usernameOrEmail: string, pass: string): Promise<User> {
    ...
    // 1. Check Firebase Realtime Database for stored credentials
    const dbUser = await UserDatabaseService.verifyCredentials(input, pass);
    ...
    if (dbUser) {
      ...
      await UserDatabaseService.saveUserToRealtimeDB(user, 'password', pass);
    ...
    // 3. Also check RTDB by email (user might exist but password didn't match)
  ...
  public static async signInWithGoogleFirebase(): Promise<User> {
    try {
      ...
      console.warn('Firebase Google Auth Popup notice:', err?.message);
      const email = 'kasukurthiavinash16@gmail.com';
      const name = 'Avinash Kasukurthi (Google Verified)';
      ...
      if (existingDbUser) {
        user = {
          ...
          email: existingDbUser.email,
          role: existingDbUser.role,
          avatarUrl: existingDbUser.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150',
          organizationId: existingDbUser.organizationId || 'org-1',
          status: 'active',
          lastLogin: new Date().toISOString(),
        };
  ...
  /**
   * Password Reset Recovery Handler
   */
  public static async requestPasswordReset(email: string): Promise<{ success: boolean; message: string }> {
    ...
    const resetUrl = `${window.location.origin}/reset-password?token=${token}&email=${encodeURIComponent(cleanEmail)}`;

    await EmailService.sendPasswordResetEmail(cleanEmail, resetUrl);

    StorageRepository.logAuditEvent({
      actorId: 'system',
      actorName: 'System Recovery',
      actorRole: 'admin',
      action: 'PASSWORD_RESET_REQUESTED',
      entityType: 'user',
      entityId: cleanEmail,
      details: `Dispatched password reset recovery link to ${cleanEmail}`,
    });
